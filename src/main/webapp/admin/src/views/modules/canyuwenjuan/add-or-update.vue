<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'" label="回复文件" prop="huifuwenjian">
          <file-upload
          tip="点击上传回复文件"
          action="file/upload"
          :limit="1"
          :multiple="true"
          :fileUrls="ruleForm.huifuwenjian?ruleForm.huifuwenjian:''"
          @change="huifuwenjianUploadChange"
          ></file-upload>
        </el-form-item>  
        <div v-else>
          <el-form-item v-if="ruleForm.huifuwenjian" label="回复文件" prop="huifuwenjian">
            <el-button type="text" size="small" @click="download(ruleForm.huifuwenjian)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
      <el-col :span="12">
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.biaoti = true;
          ...
          if(o=='huifuwenjian'){
            this.ruleForm.huifuwenjian = obj[o];
	    this.ro.huifuwenjian = true;
            continue;
          }
          if(o=='yijianjianyi'){
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `canyuwenjuan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.canyuwenjuanCrossAddOrUpdateFlag = false;
                  this.parent.search();
    ...
    huifuwenjianUploadChange(fileUrls) {
	this.ruleForm.huifuwenjian = fileUrls;
			this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
