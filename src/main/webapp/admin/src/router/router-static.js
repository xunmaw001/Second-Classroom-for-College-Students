import Vue from 'vue';
//配置路由
import VueRouter from 'vue-router'
Vue.use(VueRouter);
//1.创建组件
import Index from '@/views/index'
import Home from '@/views/home'
import Login from '@/views/login'
import NotFound from '@/views/404'
import UpdatePassword from '@/views/update-password'
import pay from '@/views/pay'
import register from '@/views/register'
import center from '@/views/center'
    import discusshuodongchengguo from '@/views/modules/discusshuodongchengguo/list'
    import xuesheng from '@/views/modules/xuesheng/list'
    import huodongbaoming from '@/views/modules/huodongbaoming/list'
    import canyuwenjuan from '@/views/modules/canyuwenjuan/list'
    import zhuanyeleixing from '@/views/modules/zhuanyeleixing/list'
    import huodongchengguo from '@/views/modules/huodongchengguo/list'
    import wenjuandiaocha from '@/views/modules/wenjuandiaocha/list'
    import huodongxinxi from '@/views/modules/huodongxinxi/list'
    import config from '@/views/modules/config/list'
    import huodongleixing from '@/views/modules/huodongleixing/list'
    import xuefenchengji from '@/views/modules/xuefenchengji/list'


//2.配置路由   注意：名字
const routes = [{
    path: '/index',
    name: '首页',
    component: Index,
    children: [{
      // 这里不设置值，是把main作为默认页面
      path: '/',
      name: '首页',
      component: Home,
      meta: {icon:'', title:'center'}
    }, {
      path: '/updatePassword',
      name: '修改密码',
      component: UpdatePassword,
      meta: {icon:'', title:'updatePassword'}
    }, {
      path: '/pay',
      name: '支付',
      component: pay,
      meta: {icon:'', title:'pay'}
    }, {
      path: '/center',
      name: '个人信息',
      component: center,
      meta: {icon:'', title:'center'}
    }
      ,{
	path: '/discusshuodongchengguo',
        name: '活动成果评论',
        component: discusshuodongchengguo
      }
      ,{
	path: '/xuesheng',
        name: '学生',
        component: xuesheng
      }
      ,{
	path: '/huodongbaoming',
        name: '活动报名',
        component: huodongbaoming
      }
      ,{
	path: '/canyuwenjuan',
        name: '参与问卷',
        component: canyuwenjuan
      }
      ,{
	path: '/zhuanyeleixing',
        name: '专业类型',
        component: zhuanyeleixing
      }
      ,{
	path: '/huodongchengguo',
        name: '活动成果',
        component: huodongchengguo
      }
      ,{
	path: '/wenjuandiaocha',
        name: '问卷调查',
        component: wenjuandiaocha
      }
      ,{
	path: '/huodongxinxi',
        name: '活动信息',
        component: huodongxinxi
      }
      ,{
	path: '/config',
        name: '轮播图管理',
        component: config
      }
      ,{
	path: '/huodongleixing',
        name: '活动类型',
        component: huodongleixing
      }
      ,{
	path: '/xuefenchengji',
        name: '学分成绩',
        component: xuefenchengji
      }
    ]
  },
  {
    path: '/login',
    name: 'login',
    component: Login,
    meta: {icon:'', title:'login'}
  },
  {
    path: '/register',
    name: 'register',
    component: register,
    meta: {icon:'', title:'register'}
  },
  {
    path: '/',
    name: '首页',
    redirect: '/index'
  }, /*默认跳转路由*/
  {
    path: '*',
    component: NotFound
  }
]
//3.实例化VueRouter  注意：名字
const router = new VueRouter({
  mode: 'hash',
  /*hash模式改为history*/
  routes // （缩写）相当于 routes: routes
})

export default router;
